...
def main():
    ...
    try:
        ...
        return
    locs, haps, seqs, cnts, cnt = [], [], [], [], 0 #locus names, grouped loci
    ...
    for line in open(sys.argv[2]): #go thru fasta file, store data and list of all loci
        if line.startswith('>'):
            locs.append(line), haps.append(line.split('_')[0]+'_'+line.split('_')[1]), cnts.append(cnt)
            cnt += 1
        else:
            seqs.append(line)

    cnt, taxCnt, subseqs, sublocs, tmpTaxa = 0, 0, [], [], []

    #process all loci & id which taxa are missing
    for hap in haps:
        if locs[cnt].split(' ')[1][1:-2] not in tmpTaxa and hap == prev:
            tmpTaxa.append(locs[cnt].split(' ')[1][1:-2]), subseqs.append(seqs[cnt]), sublocs.append(locs[cnt])
            taxCnt += 1
